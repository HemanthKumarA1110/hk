#!/usr/bin/env python3
"""Fast tuning for SCALP-SMC-006 on last ~60 days Angel One 1m.

Usage (inside strategy-engine container):

    python /app/scripts/tune_scalp_smc006.py
...
def main() -> int:
    ...
    from trading_shared.strategies.scalping_desk.smc_orb_fvg_tuning import (
        SMC_ORB_FVG_BANK_LEGACY,
        score_smc_orb_fvg_backtest,
        smc_orb_fvg_candidate_grid,
    )
    ...
    for idx, smc in enumerate(smc_orb_fvg_candidate_grid()):
        ...
        score = score_smc_orb_fvg_backtest(result)
        label = "legacy" if smc == SMC_ORB_FVG_BANK_LEGACY else f"v{idx}"
        print(
