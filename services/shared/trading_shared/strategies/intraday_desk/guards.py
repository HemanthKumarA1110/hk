"""Trading guards for intraday desk auto-trading."""
...
MARKET_OPEN_HOUR = 9
MARKET_OPEN_MIN = 15
NO_ENTRY_AFTER = (15, 0)  # 3:00 PM — no new entries
FORCE_EXIT_AFTER = (15, 15)  # 3:15 PM flat
...
def must_force_exit() -> bool:
    now = _now_ist()
    if now.weekday() >= 5:
        return True
    t = now.hour * 60 + now.minute
    cutoff = FORCE_EXIT_AFTER[0] * 60 + FORCE_EXIT_AFTER[1]
    return t >= cutoff
...
def guard_status(state: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "must_force_exit": must_force_exit(),
